...
nSamp = 65;
xMin = [1:65] - (nSamp/2 + 1);
g1 = mkGaussKernel([1 65],[1 1]);
g2 = mkGaussKernel([1 65],[1 5]);
ls = g1 - 0.3*g2;
plot(xMin,ls)
grid on
xlabel('Distance (arc min)')
ylabel('Responsivity')
print -deps ls.eps
...
colormap(cool(32))
g1 = mkGaussKernel([65 65],[3 3]);
g2 = mkGaussKernel([65 65],[10 10]);
ps = g1 - 0.6*g2;
surf(ps)
xlabel('X min'),ylabel('Y min')
grid on
print -depsc ps.eps
