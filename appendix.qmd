...
$$
\\lspreadhati{i+N} = \\lspreadi{i} ~~~\\mbox{and}~~~ \\pixvechati{i+N}=\\pixveci{i} .
...
Beginning with our analysis of optics in [Chapter 2](/chapter-2-image-formation/), we have relied on the fact that the response of a shift-invariant system to a harmonic function at frequency $f$ is also a harmonic function at $f$. In that chapter, the result was stated in two equivalent ways.
...
We can derive this result from the convolution formula. Define a new variable, $k=i-j$, and substitute $k$ into Equation~[6](#id2513666426). Remember that the summation can take place over any adjacent $N$ values. Hence, the substitution yields a modified convolution formula,

$$
\resvechati{i} = \sumoverNk \lspreadhati{k} \pixvechati{ {i - k} }
$$ {#eq-convolution-shift}

Next, we use the convolution formula in Equation~[7](#eq-convolution-shift) to compute the response to a sinusoidal input $\SfNj$. From trigonometry we have that
...
Substitute Equation~[8](#eq-sin-angle-sum) into Equation~[7](#eq-convolution-shift), remembering that $\sin (-k) = - \sin (k)$ and $\cos (-k) = \cos (k)$.

$$
\resvechati{i} = \sumoverNk \lspreadhati{k} \SfNi \CfNk + \sumoverNk \lspreadhati{k} \SfnNk \CfNi \nonumber \\
= \SfNi \sumoverNk \lspreadhati{k} \CfNk - \CfNi \sumoverNk \lspreadhati{k} \SfNk
...
$$
\left ( \begin{array}{c} ~ \\ \pixvec \\ ~ \end{array} \right ) = 
\left ( \begin{array}{ccc} ~ & ~ & ~ \\ ~ & \matCos & ~\\ ~ & ~ & ~ \end{array} \right ) 
\left ( \begin{array}{c} ~ \\ \vect{a} \\ ~ \end{array} \right ) + 
\left ( \begin{array}{ccc} ~ & ~ & ~ \\ ~ & \matSin & ~\\ ~ & ~ & ~ \end{array} \right ) 
\left( \begin{array}{c} ~ \\ \vect{b} \\ ~ \end{array} \right )
$$ {#eq-dfs-tableau}
...
$$
\vect{a} = \frac{1}{N} \matCos^T \pixvec ~~~\mbox{and}~~~ \vect{b} = \frac{1}{N} \matSin^T \pixvec .
$$ {#eq-dfs-coefficients}

We can derive the relationship in Equation~[15](#eq-dfs-coefficients) from two observations. First, the matrix sum, $\matr{H} = \matSin + \matCos$ has a simple inverse. The columns of $\matr{H}$ are orthogonal to one another, so that the inverse of $\matr{H}$ is simply [^3].
...
We can use these two observations to derive Equation~[15](#eq-dfs-coefficients) as follows. Express the fact that $\matr{H}$ and $\frac{1}{N} \matr{H}^{T}$ are inverses as follows:

$$
{{\bf I}_{N \times N}} = \matr{H} \left(\frac{1}{N} \matr{H}^T \right) = \frac{1}{N} (\matCos + \matSin) (\matCos + \matSin)^T = \frac{1}{N} (\matCos \matCos^T + \matSin \matSin^T)
$$ {#eq-dfs-inverse-proof}

where ${\matr{I}_{N \times N}}$ is the identity matrix. Then, multiply both sides of Equation~[16](#eq-dfs-inverse-proof) by $\pixvec$,

%cn cn’ stim + sn sn’ stim = stim.

$$
\pixvec = \frac{1}{N} (\matCos \matCos^T \pixvec + \matSin \matSin^T \pixvec )
$$ {#eq-dfs-reconstruction}

Compare Equation~[17](#eq-dfs-reconstruction) and Equation~[13](#eq-dfs-tableau}. Notice that the Equations become identical if we make the assignments in Equation~[15](#eq-dfs-coefficients). This completes the sketch of the proof.
...
Next, we can use our knowledge of the response to sinusoids and cosinusoids at $f$ to predict the response to any harmonic function at $f$. Suppose that the input is a harmonic function $a \CfNi + b \SfNi$, and the output is, say $a^\prime \CfNi + b^\prime \SfNi$. If the response to a unit amplitude cosinusoid is $u_f \CfNi + v_f \SfNi$, then the response to a unit amplitude sinusoid is $v_f \CfNi - u_f \SfNi$. Using these two facts and linearity we calculate the coefficients of the response,
...
More generally, if we set the phosphors to the three relative intensities $\mathbf{e} = ( \er , \eg , \eb )$, the light emitted by the CRT will be the weighted sum

$$
\mathbf{t} = \er \mathbf{m}_{1} + \eg \mathbf{m}_{2} + \eb \mathbf{m}_{3}
...
We use two basic matrices in these calculations. We have already created the matrix $\mathbf{M}$ whose three columns contain the spectral power distributions of the phosphors at maximum intensity. We also create a matrix, $\Iodopsin$, whose three columns contain the cone absorption sensitivities ($L$, $M$, and $S$), measured through the cornea and lens (see Table~??.) Given a set of relative intensities, $\mathbf{e}$, we calculate the relative cone photopigment absorption rates, $\lms$ by the matrix product <sup>4</sup>

$$
\lms = \Iodopsin^T \mathbf{M} \mathbf{e} = \Calibration \mathbf{e} .
$$ {#eq-calibration-matrix}

We call the matrix $\Calibration = \Iodopsin^T \mathbf{M}$, the monitor’s *calibration matrix.* This matrix relates the linear phosphor intensities to the relative cone absorption rates.
...
Suppose we wish to display a pair of stimuli that differ only in their effects on the $ S $ cones. Let’s begin with a stimulus whose relative intensities are $ \mathbf{e} = (0.5,0.5,0.5)^T $. Using the calibration matrix, we calculate that the relative cone absorption rates from this stimulus: they are $ (.706, .656, .558) $. Now, let’s create a second stimulus with a slightly higher rate of $ S $ cone absorptions: say, $ \lms = (.706, .656, .700) $. Using the inverse calibration matrix, we can calculate the relative display intensities needed to produce the second stimulus: they are $ (0.555, 0.4634, 0.645) $.
...
$$
P(\stimA \mid \data ) = P ( \data \mid \stimA ) \frac{ P ( \stimA ) }{ P ( \data ) }
$$ {#eq-bayes-rule}

where $P(\stimA)$ is the probability the experimenter presents signal $\stimA$ (in this case one-half) and $P(\data)$ is the probability of observing $\data$ quanta. As we shall see, this second probability turns out to be irrelevant to our decision.
...
$$(\mbox{If}~~ P(\data \mid \stimA) \frac { P(\stimA)}{ P(\data) } > P(\data \mid \stimB) \frac{ P(\stimB) }{ P(\data) } ~~\mbox{choose $\stimA$,~else~$\stimB$}.)
$$ {#eq-bayes-decision-1}

Since the probability of observing the data, $P(\data)$, divides both sides of this inequality, this probability is irrelevant to the
decision. Therefore, we can re-write Equation [22](#id1391374113) as

<a name="id3514090741"></a>

$$
\frac{ P(\data \mid \stimA) }{ P(\data \mid \stimB) } > \frac{ P(\stimB) } { P(\stimA) }.
$$ {#eq-bayes-decision-2}

The term on the left hand side of Equation~[23](#id3514090741) is called the *likelihood ratio*. The quantity on the right is called the *odds ratio*. The formula tells us that we should select $\stimA$ when the likelihood ratio, which depends on the data, exceeds the odds ratio, which depends on the a priori knowledge. A system that uses this formula to classify the stimuli is called a *Bayes classifier*.
...
$$
\mbox{If}~~\frac{P(\data \mid \stimA)}{P(\data \mid \stimB)} > \frac{P( \stimB )}{P( \stimA )}~~ \mbox{choose A, else B}.
...
$$
\mbox{If}~~P( \data \mid \stimA ) > P( \data \mid \stimB )~~ \mbox{choose $\stimA$, else $\stimB$}.
$$ {#eq-bayes-classifier-equal-prior}

Next, we can use independence to write

$$
\mbox{If}~~\prod_{i=1}^N P(d_i \mid \stimA) > \prod_{i=1}^N P(d_i \mid \stimB)~~\mbox{choose A, else B}.
...
$$
\mbox{If}~ \sum_{i=1}^{N} d_i \ln \left(\frac{\muAi}{\muBi}\right) > \sum_{i=1}^{N} \muAi - \sum_{i=1}^{N} \muBi~ \\
\mbox{choose A, else B}.
$$ {#eq-bayes-classifier-poisson}

Equation~[28](#eq-bayes-classifier-poisson) can be interpreted as a very simple computational procedure. First, notice that the Equation contains two terms. The first term is the weighted sum of the photopigment absorptions,

$$
\sum_{i=1}^{N} d_i w_i ,
$$ {#eq-bayes-classifier-weighted-sum}

where $w_i= \ln \left( \frac{\muAi}{\muBi} \right)$. This is a very familiar computation; it is directly analogous to the calculation implemented by a linear neuron whose receptive field sensitivity at position $i$ is $w_i$ (see [Chapter 5](/chapter-5-the-retinal-representation/)).
...
This term acts as a normalizing criterion to correct for the overall response to the two stimuli. The decision rule in Equation~[28](#eq-bayes-classifier-poisson) compares a weighted sum of cone absorptions to the normalizing criterion. If the first term exceeds the second, then choose response $\stimA$, else $\stimB$.
...
> <sup>6 </sup><small>Physicists often refer to Equation~[30](#eq-dot-product) as the scalar product because the result is a scalar; mathematicians often refer to the dot product as an inner product and write it using angle brackets $\langle \mathbf{a}, \mathbf{x} \rangle$.</small>
...
> <sup>7 </sup><small>In general, we would place the perpendicular line at a distance of $\| \vect{x} \| \cos ( \theta_i ) / \| \vect{a}_i\|$, but we have assumed that $\vect{a}_i$ has unit length.</small>
...
$$
u = ( \vecti{p}{1} / \vecti{p}{3}) f ~~~\mbox{and}~~~ v = (\vecti{p}{2} / \vecti{p}{3} ) f .
...
We can compute the change in the coordinate, $\delta \vect{p} = (\delta \vecti{p}{1}, \delta \vecti{p}{2}, \delta \vecti{p}{3})^T$, given the translation, rotation, and point coordinates as follows.

$$
\begin{aligned}
\delta \vecti{p}{1} & = \vecti{r}{z} \vecti{p}{2} - \vecti{r}{y} \vecti{p}{3} - \vecti{t}{1} \\
\delta \vecti{p}{2} & = \vecti{r}{x} \vecti{p}{3} - \vecti{r}{z} \vecti{p}{1} - \vecti{t}{2} \\
\delta \vecti{p}{3} & = \vecti{r}{y} \vecti{p}{1} - \vecti{r}{x} \vecti{p}{2} - \vecti{t}{3}
\end{aligned}
...
Equation~[35](#eq-motion-flow) defines how the motion of the pinhole camera and the depth map combine to create the motion flow field (Heeger and Jepson, 1992).

$$
\motFlow{u}{v} = \frac{1}{\vecti{p}{3}(u,v)} \matr{A}(u,v) \vect{t} + \matr{B}(u,v) \vect{r} .
$$ {#eq-motion-flow}
